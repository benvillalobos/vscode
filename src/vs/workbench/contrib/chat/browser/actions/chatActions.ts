...
export const GENERATE_INSTRUCTIONS_COMMAND_ID =
	"workbench.action.chat.generateInstructions";
export const GENERATE_INSTRUCTION_COMMAND_ID =
...
export function registerChatActions() {
	...
	registerAction2(
		class ToggleChatAction extends Action2 {
			...
			async run(accessor: ServicesAccessor) {
				...
				const clickBehavior =
					configurationService.getValue<AgentsControlClickBehavior>(
			...
			constructor() {
				super({
					...
					shortTitle: localize2(
						...
						"Generate Instructions with Agent",
					),
					category: CHAT_CATEGORY,
		...
		class GenerateInstructionAction extends Action2 {
			constructor() {
				super({
					...
					shortTitle: localize2(
						"generateInstruction.short",
						"Generate Instruction with Agent",
					),
					category: CHAT_CATEGORY,
					icon: Codicon.sparkle,
					f1: true,
	...
	registerAction2(
		class RenameChatAction extends Action2 {
			constructor() {
				super({
					id: "workbench.action.chat.rename",
					title: localize2("chat.rename.label", "Rename Chat..."),
					category: CHAT_CATEGORY,
					icon: Codicon.edit,
					f1: true,
					precondition: ContextKeyExpr.and(
			...
			async run(accessor: ServicesAccessor, ...args: unknown[]) {
				...
				const context = isChatViewTitleActionContext(args[0])
					? args[0]
					: undefined;
				...
				const currentTitle = widget?.viewModel?.model.title ?? "";

				const title = await quickInputService.input({
					prompt: localize("chat.rename.prompt", "New chat title"),
					value: currentTitle,
				});
				if (title) {
