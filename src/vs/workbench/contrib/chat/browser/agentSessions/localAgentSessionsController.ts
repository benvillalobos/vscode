	...
	implements IChatSessionItemController, IWorkbenchContribution
{
	...
	private registerListeners(): void {
		...
		this._register(
			this.chatSessionsService.registerChatModelChangeListeners(
				this.chatService,
		...
		this._register(
			this.chatService.onDidDisposeSession((e) => {
				const session = e.sessionResource.filter(
		...
		this._register(
			this.chatService.onDidChangeChatSessionTitle((e) => {
	...
	private toChatSessionItem(chat: IChatDetail): IChatSessionItem | undefined {
		...
		if (model) {
			...
			description =
				this.chatSessionsService.getInProgressSessionDescription(model);
		}
		...
		return {
			...
			status: model
				...
				: this.chatResponseStateToStatus(chat.lastResponseState),
	...
	private modelToStatus(model: IChatModel): ChatSessionStatus | undefined {
		if (model.requestInProgress.get()) {
			this.logService.trace(
				`[agent sessions] Session ${model.sessionResource.toString()} request is in progress.`,
			);
		...
		this.logService.trace(
			`[agent sessions] Session ${model.sessionResource.toString()} last request response: state ${lastRequest?.response?.state}, isComplete ${lastRequest?.response?.isComplete}, isCanceled ${lastRequest?.response?.isCanceled}, error: ${lastRequest?.response?.result?.errorDetails?.message}.`,
		);
