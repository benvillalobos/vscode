...
The automation dialog uses `NewChatInputWidget` as its sole composer. The widget wraps the independent `ISessionsManagementService.automationSession` draft locally, owns the prompt used for validation and save, and disables send, attachments, history persistence, and global new-session draft state. Mirroring `NewChatWidget`, the dialog owns the workspace picker and renders it beside the widget's authoritative session-type picker in a compact row; workspace selection updates the dialog's isolation model, which recreates the automation-session draft consumed by the input. The widget's provider-contributed `NewSessionControl` toolbar follows those pickers, preserving its scoped session context while exposing controls such as permissions. The dialog-owned Worktree toggle and branch picker remain one combined isolation control and render at the row's trailing edge. Editing applies the persisted provider snapshot before controls render; schema-v3 model/mode/permission fields are first translated by the selected provider. Saving captures one settled provider snapshot from the draft. The dialog retains the previous draft until a replacement target passes trust and discards its exact draft when the target becomes incomplete or the dialog closes.
...
Providers can expose `automationConfiguration` to capture, validate, and fully apply an opaque, versioned snapshot of their session-draft configuration. Capture waits for draft configuration resolution, snapshots are bound to the exact provider/session-type target stored on the Automation, and validation canonicalizes provider-owned values against current policy. Schema-v4 Automation ledgers persist this snapshot; schema-v3 model/mode/permission fields remain readable and are used only when no snapshot exists. Headless Automation execution applies the snapshot before generic isolation/branch targeting and before the first turn, with provider configuration taking precedence over legacy model/mode/permission fields.
