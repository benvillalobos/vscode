...
The Automation dialog uses `NewChatInputWidget` as its sole composer. It wraps the independent `ISessionsManagementService.automationSession` draft, omits the send button, and disables persisted new-session input state so it cannot submit requests or overwrite the global new-session draft.
