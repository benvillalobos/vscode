...
- **`ISessionsProvider`** (`sessionsProvider.ts`) — Contract every provider implements. Covers workspace discovery, session CRUD, sending requests, model enumeration/selection/presentation (`getModelsSnapshot`, `getModelPickerOptions`, `onDidChangeModels`, `setModel`), optional automation definitions and provider-owned configuration (`automation`), and firing change events.
- **`ISessionsManagementService`** (`sessionsManagement.ts`) — The session **model** service. Aggregates sessions and automation definitions from all providers, owns the pending new-session draft (`createNewSession`/`newSession`) and the independent automation-dialog draft (`createAutomationSession`/`createAutomationQuickChat`/`automationSession`/`discardAutomationSession`), routes automation configuration capture/resolution/application, sends (`sendNewChatRequest`/`createAndSendNewChatRequest`/`sendRequest`), performs CRUD (archive/delete/rename), and tracks recency history. It performs **no** view/layout mutation and never imports the view or part service. It does **not** own the active session — that lives in the view service.
...
The automation dialog uses `NewChatInputWidget` as its sole composer. The widget wraps the independent `ISessionsManagementService.automationSession` draft locally, owns the prompt used for validation and save, disables its send button, and disables persisted new-session input state, so mounting it cannot submit requests or read or overwrite the global new-session composer draft. Mirroring `NewChatWidget`, the dialog owns the workspace picker and renders it beside the widget's authoritative session-type picker in a compact row; workspace selection updates the dialog's isolation model, which recreates the automation-session draft consumed by the input. The widget's provider-contributed `NewSessionControl` toolbar follows those pickers, preserving its scoped session context while exposing controls such as permissions. The dialog publishes the automation draft's session context keys into that same scope, so provider-control visibility follows the selected automation target rather than the globally active session. The dialog-owned Worktree toggle and branch picker remain one combined isolation control and render at the row's trailing edge. The dialog synchronizes its workspace-backed automation draft after workspace or session-type changes settle, retains the previous draft until the replacement target passes trust, and discards its exact draft when the target becomes incomplete, switches to quick chat, or the dialog closes. Quick-chat draft creation remains separate.

Automation definitions and their configuration belong to the selected sessions provider. A provider advertises stable definitions for exact session-type IDs through `ISessionsProvider.automation`, captures an opaque versioned JSON-compatible configuration from its draft, resolves or migrates persisted configuration, and applies it before a scheduled prompt is sent. Model configuration such as reasoning effort and context size is part of that provider snapshot rather than implied by the model identifier or read from mutable global configuration at run time. The shared automation ledger persists the provider/session-type target, definition ID, schedule, prompt, run history, and opaque configuration; it does not interpret provider fields such as model, mode, permissions, or Agent Host dynamic session configuration. `ISessionsManagementService` preserves exact provider identity while aggregating definitions and remains the trusted execution path for workspace trust, cancellation, draft cleanup, sending, and lifecycle observation.
