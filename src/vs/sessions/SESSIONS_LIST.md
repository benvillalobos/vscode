...
Automation run history uses `SessionsFlatList` for runs backed by a live session. Pending and running runs without a resolved session use a lightweight `Working...` row; date grouping, pagination, and run actions remain owned by the Automations view.
