...
class AutomationsCustomViewAccessibilityHelp implements IAccessibleViewImplementation {
	...
	getProvider(accessor: ServicesAccessor): AccessibleContentProvider {
		...
		const content = [
			...
			localize('automationsCustomView.help.cron', "In the automation dialog, choose Cron to enter minute, hour, day of month, month, and day of week. Tab moves between the five parts. Up and Down step simple numeric values. Select All then Copy copies the entire expression; pasting a full expression fills the parts. Blank parts mean every value. Entirely empty input has no error, but cannot be saved."),
