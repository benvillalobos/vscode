...
class AutomationsCustomViewAccessibilityHelp implements IAccessibleViewImplementation {
	...
	getProvider(accessor: ServicesAccessor): AccessibleContentProvider {
		...
		const content = [
			...
			localize('automationsCustomView.help.deleteRunning', "An automation cannot be deleted while it is running. Wait for the run to finish before deleting the automation."),
			localize('automationsCustomView.help.cards', "For saved automations, Tab to a card's Edit control and action buttons. Use Left Arrow and Right Arrow to move between Run now and More Actions. Press Enter or Space to activate a control. Edit, or clicking anywhere else on the card, opens the automation dialog. More Actions opens the automation menu. Duplicate opens a prefilled New automation dialog, Disable prevents scheduled runs, and Delete asks for confirmation. Run now starts a session immediately."),
