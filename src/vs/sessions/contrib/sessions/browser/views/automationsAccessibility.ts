...
class AutomationsCustomViewAccessibilityHelp implements IAccessibleViewImplementation {
	...
	getProvider(accessor: ServicesAccessor): AccessibleContentProvider {
		...
		const content = [
			...
			localize('automationsCustomView.help.history', "Run history is grouped by date. Each run reports its Pending, Running, Completed, or Failed status. Tab to a run's Open Session button and press Enter or Space to open its available session. The button is disabled when no session is available. Tab to a run's delete button to permanently delete its session and history item, or to remove a terminal run that has no session from history, after confirmation."),
