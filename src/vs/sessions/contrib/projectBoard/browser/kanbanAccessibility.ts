...
class KanbanAccessibilityHelp implements IAccessibleViewImplementation {
	...
	getProvider(accessor: ServicesAccessor): AccessibleContentProvider {
		const content = [
			...
			localize('kanban.help.editing', "Use the custom view header buttons to add rows or columns, show archived chats, start a new session, or change board display settings. Use the row and column controls on the board to rename, reorder, or remove axes. Use a column's collapse button to hide or show its sessions. Drag a card onto a cell to move it."),
