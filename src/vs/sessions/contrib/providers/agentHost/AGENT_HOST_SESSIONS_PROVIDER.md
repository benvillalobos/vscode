...
Agent Host automation configuration captures the selected model's full configuration, including reasoning effort and context size, alongside its identifier. Editing preserves that snapshot unless the user changes the model configuration in the dialog, and headless execution passes the snapshot explicitly so concurrent automation runs do not depend on or mutate the current global model configuration.
