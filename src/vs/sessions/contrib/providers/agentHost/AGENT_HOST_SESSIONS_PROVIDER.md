...
Agent Host Automation configuration snapshots capture the selected model and the resolved create-session configuration only after the untitled draft finishes resolving. Snapshots carry the exact provider/session-type identity, validation deep-canonicalizes JSON values and reapplies current configuration policy, and application fully replaces the draft configuration with rollback on resolution failure. Automated first turns send the resulting configuration as a replacement for eager backend sessions so omitted keys cannot survive from an earlier draft state.
