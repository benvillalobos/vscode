...
suite("Sticky Scroll Tests", () => {
	...
	test("preferInnerScopes: false (default) - outer scopes shown when limit exceeded", () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			const model = createTextModel(textWithScopesWithSameStartingLines);
			await withAsyncTestCodeEditor(
				model,
				...
				async (editor, _viewModel, instantiationService) => {
					...
					// Line 1 = TestClass+foo, line 2 = bar. With maxLineCount=1 and preferInnerScopes=false, only the outermost (line 1) is shown
					editor.setScrollTop(1);
	...
	test("preferInnerScopes: true - inner scopes shown when limit exceeded", () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			const model = createTextModel(textWithScopesWithSameStartingLines);
			await withAsyncTestCodeEditor(
				model,
				...
				async (editor, _viewModel, instantiationService) => {
					...
					// With preferInnerScopes=true, the innermost scope (line 2 = bar) is shown instead
					editor.setScrollTop(1);
					const state = stickyScrollController.findScrollWidgetState();
					assert.deepStrictEqual(state.startLineNumbers, [2]);
	...
	test("preferInnerScopes: true with maxLineCount=2 trims only outer scopes", () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			// Uses the second test model (TestClass > foo > bar) with maxLineCount=2.
			// TestClass(1-5), foo(1-4), bar(2-4) => at scrollTop=1, candidates are [1, 2]
			// With maxLineCount=2, all fit, so no trimming occurs
			const model = createTextModel(textWithScopesWithSameStartingLines);
			await withAsyncTestCodeEditor(
				model,
				{
				...
				async (editor, _viewModel, instantiationService) => {
					...
					// At scrollTop=1, 2 scopes match (line 1 and line 2), maxLineCount=2 fits them all
					editor.setScrollTop(1);
					let state = stickyScrollController.findScrollWidgetState();
					assert.deepStrictEqual(state.startLineNumbers, [1, 2]);
	...
	test("preferInnerScopes: false with maxLineCount=2 keeps outer scopes", () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			const model = createTextModel(textWithScopesWithSameStartingLines);
			await withAsyncTestCodeEditor(
				model,
	...
	test("preferInnerScopes: no trimming when scopes fit within maxLineCount", () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			const model = createTextModel(textWithScopesWithSameStartingLines);
			await withAsyncTestCodeEditor(
				model,
				...
				async (editor, _viewModel, instantiationService) => {
					...
					// With maxLineCount=5, all scopes fit — preferInnerScopes has no effect
					editor.setScrollTop(1);
					const state = stickyScrollController.findScrollWidgetState();
					assert.deepStrictEqual(state.startLineNumbers, [1, 2]);
	...
	test("preferInnerScopes: at different scroll positions", () => {
		return runWithFakedTimers({ useFakeTimers: true }, async () => {
			const model = createTextModel(textWithScopesWithSameStartingLines);
			await withAsyncTestCodeEditor(
				model,
				...
				async (editor, _viewModel, instantiationService) => {
					...
					// At scrollTop=1, bar scope (line 2) is visible -> show inner scope
					editor.setScrollTop(1);
					let state = stickyScrollController.findScrollWidgetState();
					assert.deepStrictEqual(state.startLineNumbers, [2]);

					// After bar scope ends (scroll past line 3), only TestClass (line 1) remains
					editor.setScrollTop(2 * lineHeight + 1);
					state = stickyScrollController.findScrollWidgetState();
					assert.deepStrictEqual(state.startLineNumbers, [1]);

					// After TestClass scope ends, no sticky lines
					editor.setScrollTop(4 * lineHeight + 1);
					state = stickyScrollController.findScrollWidgetState();
					assert.deepStrictEqual(state.startLineNumbers, []);
